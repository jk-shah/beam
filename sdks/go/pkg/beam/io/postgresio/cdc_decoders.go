...
// binaryArrayElemWidth gives the exact byte width of the fixed-size element
// types that DecodeBinaryArray decodes by direct indexing. Types absent from
// this table are variable width and are handled without a length assumption.
var binaryArrayElemWidth = map[int32]int{
	23:  4, // INT4
	20:  8, // INT8
	701: 8, // FLOAT8
	700: 4, // FLOAT4
...
func DecodeBinaryArray(data []byte) ([]any, error) {
	...
	// dimLen sizes an allocation from a wire-supplied int32. Each element costs
	// at least its own 4-byte length prefix, so the bytes remaining bound it
	// exactly; a negative value would panic make outright.
	if dimLen < 0 {
		return nil, fmt.Errorf("postgresio: negative array length %d", dimLen)
	}
	if int64(dimLen)*4 > int64(r.Len()) {
		return nil, fmt.Errorf("postgresio: array length %d exceeds %d bytes remaining", dimLen, r.Len())
	}
	...
	for i := 0; i < int(dimLen); i++ {
		...
		if itemLen < 0 {
			return nil, fmt.Errorf("postgresio: negative array item length %d at index %d", itemLen, i)
		}
		if int(itemLen) > r.Len() {
			return nil, fmt.Errorf("postgresio: array item length %d at index %d exceeds %d bytes remaining",
				itemLen, i, r.Len())
		}
		itemBytes := make([]byte, itemLen)
		// io.ReadFull rather than r.Read: a short read would leave the tail as
		// zeros and decode to a plausible wrong number rather than an error.
		if _, err := io.ReadFull(r, itemBytes); err != nil {
			return nil, err
		}
		// The fixed-width branches below index itemBytes directly, so an item
		// shorter than its type's width would panic with a slice bounds error.
		// The server does not send those, but a corrupted frame can.
		if want, ok := binaryArrayElemWidth[elemOID]; ok && len(itemBytes) < want {
			return nil, fmt.Errorf("postgresio: array item %d for type OID %d is %d bytes, need %d",
				i, elemOID, len(itemBytes), want)
		}
