...
func TestNegotiateProtoVersion(t *testing.T) {
	// Thresholds are from the PostgreSQL logical replication protocol
	// specification: v2 needs server 14, v3 needs 15, v4 needs 16. An earlier
	// implementation gated all of these on server 19, which does not exist, so
	// every real server fell back to v1.
	cases := []struct {
		serverMajor int
		want        int
	}{
		{0, 1}, // version detection failed
		{10, 1},
		{13, 1},
		{14, 2},
		{15, 3},
		...
		{17, 4},
		{18, 4},
		{19, 4}, // forward compatible with a future release
	}
	for _, c := range cases {
		if got := negotiateProtoVersion(c.serverMajor); got != c.want {
			t.Errorf("negotiateProtoVersion(%d) = %d, want %d", c.serverMajor, got, c.want)
		}
	}
}

func TestCDCStream_BuildStartReplicationQuery_VersionNegotiation(t *testing.T) {
	baseOpts := func(extra ...CDCOption) CDCOptions {
		return NewCDCOptions(append([]CDCOption{
			WithCDCSlotName("beam_slot"),
			WithCDCPublication("beam_pub"),
		}, extra...)...)
	}

	// Currently supported servers negotiate the protocol version they actually
	// implement. Declaring a higher version does not by itself change framing,
	// so this is safe without also enabling streaming or two_phase.
	for _, c := range []struct {
		...
		s := &NativeReplicationStream{opts: baseOpts(), serverMajorVersion: c.serverMajor}
		q := s.BuildStartReplicationQuery("0/0")
		if !strings.Contains(q, c.wantProto) {
			t.Errorf("server %d: expected %s, got: %s", c.serverMajor, c.wantProto, q)
		}
	}

	// Binary mode and streaming stay off unless asked for, so that upgrading a
	// server does not silently move every column onto the binary decode path.
	sDefault := &NativeReplicationStream{opts: baseOpts(), serverMajorVersion: 18}
	qDefault := sDefault.BuildStartReplicationQuery("0/0")
	if strings.Contains(qDefault, "binary 'true'") {
		t.Errorf("binary mode should be opt-in, got: %s", qDefault)
	}
	if strings.Contains(qDefault, "streaming") {
		t.Errorf("streaming should be opt-in, got: %s", qDefault)
	}

	// Version detection failure falls back to the baseline.
	sUnknown := &NativeReplicationStream{opts: baseOpts(), serverMajorVersion: 0}
	qUnknown := sUnknown.BuildStartReplicationQuery("0/0")
	if !strings.Contains(qUnknown, "proto_version '1'") {
		t.Errorf("unknown server version should use proto_version '1', got: %s", qUnknown)
	}

	// Requested features are honoured where the server supports them.
	sOptIn := &NativeReplicationStream{
		opts:               baseOpts(WithCDCBinaryMode(true), WithCDCStreamingMode("parallel")),
		serverMajorVersion: 16,
	}
	qOptIn, unsupported := sOptIn.buildStartReplicationQuery("0/0")
	if !strings.Contains(qOptIn, "binary 'true'") || !strings.Contains(qOptIn, "streaming 'parallel'") {
		t.Errorf("server 16 should honour binary and parallel streaming, got: %s", qOptIn)
	}
	if len(unsupported) != 0 {
		t.Errorf("server 16 should support both options, got unsupported: %v", unsupported)
	}

	// Where the server cannot support a requested feature it is omitted and
	// reported, rather than sent and rejected by the server.
	sTooOld := &NativeReplicationStream{
		opts:               baseOpts(WithCDCStreamingMode("parallel")),
		serverMajorVersion: 14,
	}
	qTooOld, unsupportedOld := sTooOld.buildStartReplicationQuery("0/0")
	if strings.Contains(qTooOld, "streaming 'parallel'") {
		t.Errorf("server 14 cannot do parallel streaming, got: %s", qTooOld)
	}
	if len(unsupportedOld) != 1 || !strings.Contains(unsupportedOld[0], "parallel") {
		t.Errorf("expected parallel streaming to be reported unsupported, got: %v", unsupportedOld)
	}

	// two_phase requires proto_version 3. Previously it was appended even at
	// proto_version 1, which made PostgreSQL reject START_REPLICATION outright.
	sTwoPhase := &NativeReplicationStream{
		opts:               baseOpts(WithCDCProtoVersion(1), WithCDCTwoPhase(true)),
		serverMajorVersion: 16,
	}
	qTwoPhase, unsupportedTP := sTwoPhase.buildStartReplicationQuery("0/0")
	if strings.Contains(qTwoPhase, "two_phase 'true'") {
		t.Errorf("two_phase must not be sent at proto_version 1, got: %s", qTwoPhase)
	}
	if len(unsupportedTP) != 1 || !strings.Contains(unsupportedTP[0], "two-phase") {
		t.Errorf("expected two-phase to be reported unsupported, got: %v", unsupportedTP)
	}

	// Origin filter is independent of version negotiation.
	sOrigin := &NativeReplicationStream{
		opts:               baseOpts(WithCDCOriginFilter("none")),
		serverMajorVersion: 16,
	}
	if q := sOrigin.BuildStartReplicationQuery("0/0"); !strings.Contains(q, "origin 'none'") {
		t.Errorf("expected origin 'none', got: %s", q)
	}

	// An explicit proto_version overrides negotiation.
	sOverride := &NativeReplicationStream{
		opts:               baseOpts(WithCDCProtoVersion(2), WithCDCBinaryMode(false), WithCDCStreamingMode("off")),
		serverMajorVersion: 18,
	}
	qOverride := sOverride.BuildStartReplicationQuery("0/0")
	if !strings.Contains(qOverride, "proto_version '2'") {
		t.Errorf("expected overridden proto_version '2', got: %s", qOverride)
	}
	if strings.Contains(qOverride, "binary 'true'") {
		t.Errorf("expected binary mode disabled by override, got: %s", qOverride)
	}
	if strings.Contains(qOverride, "streaming") {
		t.Errorf("expected streaming disabled by override, got: %s", qOverride)
	}
}

func TestCDCStream_ParameterStatus_WireNegotiation(t *testing.T) {
	// Requested options that the negotiated protocol version supports must
	// reach the wire. proto_version is raised automatically from the server
	// version, but binary and streaming are only sent when asked for.
	t.Run("PG19_SendsRequestedBinaryAndParallelStreamingAtProto4", func(t *testing.T) {
		clientConn, serverConn := net.Pipe()
		defer clientConn.Close()
		defer serverConn.Close()
		...
		opts := NewCDCOptions(
			...
			WithCDCPublication("test_pub"),
			WithCDCSSLMode("disable"), // stub server speaks plaintext
			// Requested explicitly: negotiation raises proto_version on its
			// own, but it does not turn these on for the caller.
			WithCDCBinaryMode(true),
			WithCDCStreamingMode("parallel"),
			WithCDCDialFunc(func(ctx context.Context, network, addr string) (net.Conn, error) {
				return clientConn, nil
			}),
		)
	...
	// PG18 negotiates the same proto_version 4 as any server >= 16, and omits
	// the options the caller did not request.
	t.Run("PG18_NegotiatesProto4AndOmitsUnrequestedOptions", func(t *testing.T) {
		clientConn, serverConn := net.Pipe()
		defer clientConn.Close()
		defer serverConn.Close()
		...
		go func() {
			...
			repQueryStr := string(qBody2)
			// PG18 is >= 16, so it negotiates proto_version 4. This assertion
			// previously required proto_version '1', which encoded the
			// version-gate defect: only 18 and an unreleased 19 were
			// recognized, so every server from 14 through 17 fell back to the
			// oldest protocol and lost streaming and two-phase framing.
			if !strings.Contains(repQueryStr, "proto_version '4'") {
				t.Errorf("PG18 wire query expected proto_version '4', got: %s", repQueryStr)
			}
			// Neither option was requested, so neither may appear, whatever
			// the negotiated version supports.
			if strings.Contains(repQueryStr, "binary 'true'") {
				t.Errorf("PG18 wire query should NOT contain binary 'true' when unrequested, got: %s", repQueryStr)
			}
			if strings.Contains(repQueryStr, "streaming 'parallel'") {
				t.Errorf("PG18 wire query should NOT contain streaming 'parallel' when unrequested, got: %s", repQueryStr)
			}
