...
func TestStagingTableNameIsStableAcrossCalls(t *testing.T) {
	...
	first := stagingTableName(table)
	for i := 0; i < 100; i++ {
		if got := stagingTableName(table); got != first {
...
func TestStagingTableNameDiffersPerTargetTable(t *testing.T) {
	orders := stagingTableName(`"public"."orders"`)
	payments := stagingTableName(`"public"."payments"`)
...
func TestStagingTableNameIsAValidIdentifier(t *testing.T) {
	name := stagingTableName(`"public"."orders"`)
