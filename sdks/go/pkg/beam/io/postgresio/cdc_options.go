...
type CDCOptions struct {
	...
	// MaxReconnectAttempts caps how many consecutive connection-level failures
	// are retried before failing the pipeline. Defaults to 5.
	MaxReconnectAttempts int

	// MaxReconnectElapsed caps how long the source may spend retrying
	// connection-level failures before failing the pipeline. Defaults to 0 (unlimited).
	...
	// transaction exceeds MaxSpooledBytes. Defaults to OversizedTxnFailPipeline.
	OversizedTxnPolicy OversizedTxnPolicy

	// TransactionStallTimeout bounds how long the stream may be silent while
	// a transaction is in progress before dropping the connection and retrying.
	// Defaults to 2 minutes if zero.
	TransactionStallTimeout time.Duration
}
...
func (o *CDCOptions) Validate() error {
	...
	switch o.StreamingMode {
	case "", "off", "on", "parallel":
	default:
		return fmt.Errorf("invalid streaming mode %q: must be one of \"off\", \"on\", \"parallel\"", o.StreamingMode)
	}

	// Two-phase commit is rejected rather than decoded. The parser has no
	// handler for Begin Prepare ('b'), so the changes of a prepared
	// transaction would be stamped with the preceding transaction's XID, LSN
	// and commit timestamp; and none for Rollback Prepared ('r'), so the
	// changes of a transaction that was rolled back would already have been
	// emitted downstream with nothing to retract them. Both produce wrong data
	// silently, which is worse than refusing the option.
	if o.TwoPhaseCommit {
		return fmt.Errorf("two-phase commit decoding is not supported: " +
			"prepared-transaction frames (Begin Prepare, Prepare, Commit Prepared, Rollback Prepared) " +
			"are not decoded, so enabling it would emit changes with incorrect transaction metadata and " +
			"would not retract the changes of a rolled-back prepared transaction")
	}
	return nil
}
...
// WithCDCReconnectAttempts sets the maximum number of consecutive connection
// failures to retry before failing the pipeline. Defaults to 5.
func WithCDCReconnectAttempts(attempts int) CDCOption {
	return func(o *CDCOptions) {
		o.MaxReconnectAttempts = attempts
	}
}

// WithCDCReconnectMaxElapsed sets the maximum elapsed time the source may spend
// retrying connection failures before failing the pipeline. Defaults to 0 (unlimited).
func WithCDCReconnectMaxElapsed(maxElapsed time.Duration) CDCOption {
	return func(o *CDCOptions) {
		o.MaxReconnectElapsed = maxElapsed
	}
}

// WithCDCMaxSpooledBytes sets the maximum size in bytes of streamed in-progress
// transactions buffered in worker memory before applying OversizedTxnPolicy.
func WithCDCMaxSpooledBytes(bytes int64) CDCOption {
	return func(o *CDCOptions) {
		o.MaxSpooledBytes = bytes
	}
}

// WithCDCOversizedTxnPolicy sets the policy for handling streamed transactions
// that exceed MaxSpooledBytes.
func WithCDCOversizedTxnPolicy(policy OversizedTxnPolicy) CDCOption {
	return func(o *CDCOptions) {
		o.OversizedTxnPolicy = policy
	}
}

// WithCDCTransactionStallTimeout sets the maximum duration the replication
// stream may remain silent during an open transaction before dropping the
// connection and reconnecting to failover standbys.
func WithCDCTransactionStallTimeout(d time.Duration) CDCOption {
	return func(o *CDCOptions) {
		o.TransactionStallTimeout = d
	}
}
