...
func ConfigToBeamSchema(t reflect.Type) (*pipepb.Schema, error) {
	...
	return &pipepb.Schema{
		Fields: fields,
		Id:     fmt.Sprintf("beam_schema_go_%s", t.Name()),
	}, nil
}
...
func reflectFields(t reflect.Type, depth int) ([]*pipepb.Field, error) {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		name, isSecret, isRequired, ignore := parseFieldTag(sf)
		if ignore {
			continue
		}

		fieldType, _, err := reflectFieldType(sf.Type, depth+1)
		if err != nil {
...
func parseFieldTag(sf reflect.StructField) (string, bool, bool, bool) {
	tag := sf.Tag.Get("beam")
	if tag == "-" {
	...
	return name, isSecret, isRequired, false
}
