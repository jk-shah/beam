...
type MaskCustomerPIIFn struct {
	RecordsIngested *metrics.Counter
	SSNMaskedCount  *metrics.Counter
	InvalidSSNCount *metrics.Counter
}

// NewMaskCustomerPIIFn constructs a new PII masking transform.
func NewMaskCustomerPIIFn() *MaskCustomerPIIFn {
	c1 := metrics.NewCounter("postgres_fanout", "records_ingested")
	c2 := metrics.NewCounter("postgres_fanout", "ssn_masked_count")
	c3 := metrics.NewCounter("postgres_fanout", "invalid_ssn_count")
...
func (fn *MaskCustomerPIIFn) ProcessElement(ctx context.Context, account CustomerAccount, emit func(CustomerAccount)) {
	if fn.RecordsIngested != nil {
		fn.RecordsIngested.Inc(ctx, 1)
	}

	origSSN := account.SSN
	account.SSN = MaskSSNZeroAlloc(origSSN)
	if account.SSN == "***-**-****" && origSSN != "" && origSSN != "***-**-****" {
		if fn.InvalidSSNCount != nil {
			fn.InvalidSSNCount.Inc(ctx, 1)
		}
	} else {
		if fn.SSNMaskedCount != nil {
			fn.SSNMaskedCount.Inc(ctx, 1)
		}
	}
...
func main() {
	...
	accounts := beam.ParDo(s, func(ev postgresio.ChangeEvent, emit func(CustomerAccount)) {
