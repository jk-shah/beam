...
func newHAResilienceFilterFn() *haResilienceFilterFn {
	return &haResilienceFilterFn{
		reconnectCounter: metrics.NewCounter("postgresio_cdc", "ha_failover_reconnect_total"),
	}
}
...
func (fn *haResilienceFilterFn) ProcessElement(evt postgresio.ChangeEvent, emit func(HAEventRecord)) {
	...
	id, _ := evt.After["record_id"].(int64)
	if id == 0 {
		if f, ok := evt.After["record_id"].(float64); ok {
			id = int64(f)
		}
	}
	...
	emit(HAEventRecord{
		RecordID:     id,
		Payload:      fmt.Sprintf("%v", evt.After["payload"]),
...
func main() {
	...
	records := beam.ParDo(s, newHAResilienceFilterFn(), stream)
