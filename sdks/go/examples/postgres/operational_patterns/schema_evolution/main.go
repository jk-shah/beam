...
type EvolvedOrderRecord struct {
	...
	CustomerID  int64     `json:"customer_id" db:"customer_id" beam:"customer_id"`
...
var driftCounter = metrics.NewCounter("postgresio_cdc", "schema_drift_detected_total")
...
func (fn *schemaAdaptiveTransformFn) ProcessElement(ctx context.Context, evt postgresio.ChangeEvent, emit func(EvolvedOrderRecord)) {
	...
	customerID, _ := raw["customer_id"].(int64)
	if customerID == 0 {
		if f, ok := raw["customer_id"].(float64); ok {
			customerID = int64(f)
		}
	}
...
func main() {
	...
	evolved := beam.ParDo(s, &schemaAdaptiveTransformFn{}, stream)
