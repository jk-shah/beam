...
func (fn *parseCDCOrderFn) ProcessElement(event postgresio.ChangeEvent, emit func(Order)) {
	...
	var orderID int64
	switch v := event.After["order_id"].(type) {
	case float64:
		orderID = int64(v)
	case int64:
		orderID = v
...
func main() {
	...
	default: // "sample"
		...
		now := time.Now().UTC()
		sampleOrders := []Order{
			{OrderID: 1001, CustomerID: "CUST-1", Item: "Compute Engine Server", Amount: 450.00, OrderedAt: now},
			{OrderID: 1002, CustomerID: "CUST-2", Item: "Cloud Storage Bucket", Amount: 80.00, OrderedAt: now},
			{OrderID: 1003, CustomerID: "CUST-1", Item: "BigQuery High-Memory Slot", Amount: 1200.00, OrderedAt: now},
			{OrderID: 1004, CustomerID: "CUST-3", Item: "Cloud Spanner Enterprise", Amount: 3200.00, OrderedAt: now},
			{OrderID: 1005, CustomerID: "CUST-999", Item: "Cloud DNS Record", Amount: 15.00, OrderedAt: now},
		}
