...
var (
	...
	inputTable = flag.String("input_table", "", "Optional PostgreSQL table to read catalog items from (e.g. public.products_catalog). If empty, uses sample catalog.")
)
...
func main() {
	...
	var catalogCol beam.PCollection
	if *inputTable != "" {
		...
		catalogCol = postgresio.Read(s, *inputTable, reflect.TypeOf(ProductRecord{}), readOpts)
	} else {
