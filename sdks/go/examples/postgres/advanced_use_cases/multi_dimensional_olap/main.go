...
var (
	...
	inputTable = flag.String("input_table", "", "Optional PostgreSQL table to read sales transactions from (e.g. public.sales_transactions). If empty, uses sample transactions.")
)
...
func main() {
	...
	var transactionsCol beam.PCollection
	if *inputTable != "" {
		readOpts := postgresio.NewReadOptions(
			postgresio.WithReadHost(*host),
			postgresio.WithReadPort(*port),
			postgresio.WithReadDatabase(*database),
			postgresio.WithReadUsername(*username),
			postgresio.WithReadPassword(*password),
			postgresio.WithReadSSLMode(*sslMode),
		)
		transactionsCol = postgresio.Read(s, *inputTable, reflect.TypeOf(SaleTransaction{}), readOpts)
	} else {
