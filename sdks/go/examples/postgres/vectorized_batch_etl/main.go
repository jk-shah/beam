...
var (
	...
	inputTable = flag.String("input_table", "", "Optional PostgreSQL table to read raw source records from (e.g. public.source_transactions). If empty, generates synthetic records.")
)
...
func main() {
	...
	var rawSource beam.PCollection
	if *inputTable != "" {
		...
		rawSource = postgresio.Read(s, *inputTable, reflect.TypeOf(SourceRecord{}), readOpts)
	} else {
