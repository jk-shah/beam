...
/**
 * Production Dataflow Streaming Template for replicating PostgreSQL CDC streams into Apache Iceberg
 * tables with support for database-wide, single-schema, or single-table scopes, Dead-Letter Queues,
 * and Dataflow streaming pause and resume.
 */
public class PostgreSqlToIcebergStreamingTemplate {

  private static final Logger LOG =
      LoggerFactory.getLogger(PostgreSqlToIcebergStreamingTemplate.class);
  ...
  public static void run(Options options) {
    ...
    LOG.info(
        "Configured PostgreSQL CDC streaming for {} tables: {}", targetTables.size(), targetTables);
    ...
    // 5. Write to Apache Iceberg via Managed IO
    processedEvents
        .get(validRowsTag)
        .apply(
            "WriteToIceberg",
            Managed.write(Managed.ICEBERG)
                .withConfig(
                    ImmutableMap.<String, Object>builder()
                        ...
                        .put(
                            "catalog_name",
                            options.getIcebergCatalogName() != null
                                ? options.getIcebergCatalogName()
                                : "default")
                        .put("catalog_type", options.getIcebergCatalogType())
                        .put("warehouse", options.getIcebergWarehouse())
                        .build()));

    // 6. Output DLQ Records if DLQ path is specified
