...
* Added an experimental `postgresio` connector to the Go SDK covering batch writes (staged COPY and UNNEST upserts), logical CDC replication streaming, SchemaTransforms, and Arrow record batching (Go). The connector has known defects affecting the source database and is not yet recommended for production use; see the [known limitations](https://github.com/apache/beam/blob/master/sdks/go/pkg/beam/io/postgresio/README.md#known-limitations).
