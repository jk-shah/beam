...
* Added an experimental `postgresio` connector to the Go SDK covering batch writes (staged COPY and UNNEST upserts), logical CDC replication streaming as an unbounded splittable DoFn, SchemaTransforms, and Arrow record batching (Go). The connector is experimental and has known limitations; see the [known limitations](https://github.com/apache/beam/blob/master/sdks/go/pkg/beam/io/postgresio/README.md#known-limitations).
